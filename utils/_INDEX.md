...
**已部署**：`onThisDay.md` —— 已嵌入 `Daily Note.md` 模板（`![[Helper/utils/importantDates]]` 之后），2026-06-06 验证通过。**无数据时自动隐藏 embed**（`dv.container` 加 `.otd-empty` 类 + 注入的 `:has()` CSS 规则；JS 在 render 阶段够不到 `.internal-embed`，故走 CSS）。

...
try {
    const cfgPage = dv.page("Helper/config/BiltRewardConfig");
    if (!cfgPage || !cfgPage.file || !cfgPage.file.frontmatter) {
        dv.paragraph("⚠️ Missing BiltRewardConfig.md");
        return;
    }
    ...
    if (cashPrefix) {
        ...
        rn[cashLabel] = cashLabel;
        if (!nonPct.includes(cashLabel)) nonPct.push(cashLabel);
    }

    const allKeys = [...new Set([...Object.keys(yearTotals), ...Object.keys(creditTotals)])].sort();
    if (allKeys.length === 0) {
        dv.paragraph(`📄 No Bilt reward data found for ${selectedYear}.`);
        return;
    }
